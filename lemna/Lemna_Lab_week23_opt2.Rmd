...
```
Follow this guide to complete your lab write up. You can answer the questions, which will be bolded in this document, either as an R notebook file using this Markdown document as a template, or in a separate word or text document. Pay attention to places where you will need to change the code for your specific question. 
...
If you want to more closely compare the individuals within each treatment, we can make little plots for each category instead of just one: 
...
###Options for calculating growth rates:  
The first option we'll go through uses base R and for loops. We'll start by creating empty vectors and then for each of our samples, we'll 'loop' through and calculate the change in N and t between subsequent time steps. Once we've calculated those for each sample, we can add them back to the lemna dataframe and calculate dN/dt and 1/N * dN/dt.
```{r}
